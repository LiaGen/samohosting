...
start() {
    if ! (whiptail --backtitle "S1 miniPC LCD DISPLAY TOOL for PROXMOX" --title "ЗАКРЫЛИ ЭТО ОКНО =>> ПРЕРВАЛИ УСТАНОВКУ" --yesno "Утилита для S1 miniPC экрана на Proxmox. Установка займет около 10 минут, чтобы не скучать - в режиме отладки(будет сыпать на экран, в том числе красные сообщения, не переживайте - это нормально.. Начать устаановку?" 10 58); then
      clear
