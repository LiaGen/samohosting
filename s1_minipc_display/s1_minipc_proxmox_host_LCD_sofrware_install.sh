...
start() {
    if ! (whiptail --backtitle "S1 miniPC LCD DISPLAY TOOL for PROXMOX" --title "S1 miniPC LCD DISPLAY TOOL for PROXMOX" --yesno "Будет установлен софт для работы с дисплеем S1 miniPC на Proxmox(не закрывайте\уходите из консоли до конца установки). Установка займет около 10 минут, чтобы Вам было не скучно - оставил режим отладки = будет сыпать мусор на экран, в том числе красные сообщения, не переживайте - это нормально..Начать устаановку?" 10 58); then
      clear
